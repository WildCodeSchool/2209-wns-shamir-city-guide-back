...
describe("integration/service/city.service suite of tests with database connexion", () => {
    ...
    it("Should return an error 422 Unprocessable Entity if we attempt to create a city with a localisation which already exist in database", async () => {
        ...
        try {
            ...
            if (e instanceof CustomError) {
                expect(e.message).toBe(`La ville avec la latitude ${latitude} et la longitude ${longitude} exist déjà en base de données`);
                expect(e.statusCodeClass).toBe(StatusCodeClass.CLIENT_ERROR);
                expect(e.statusCodeClass).toEqual(StatusCodeClass.CLIENT_ERROR);
                expect(e.statusCode).toBe(StatusCode.UNPROCESSABLE_ENTITY);
                expect(e.statusCode).toEqual(StatusCode.UNPROCESSABLE_ENTITY);
            ...
            expect(e).toStrictEqual(
                new CustomError(
                    new UnprocessableEntityError(), 
                    `La ville avec la latitude ${latitude} et la longitude ${longitude} exist déjà en base de données`
                ))
        }
    });
